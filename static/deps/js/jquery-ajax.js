...
$(document).ready(function () {
    ...
    $(document).on("click", ".remove-from-cart", function (e) {
        // Блокируем его стандартное действие (например, переход по ссылке)
        e.preventDefault();
    
        // Получаем элемент счётчика товаров в корзине
        var goodsInCartCount = $("#goods-in-cart-count");
        // Парсим текущее значение счётчика, если оно отсутствует — используем 0
        var cartCount = parseInt(goodsInCartCount.text() || 0);
    
        // Получаем id корзины из атрибута data-cart-id кнопки
        var cart_id = $(this).data("cart-id");
        // Получаем URL для удаления из корзины из атрибута href
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // Обновляем отображение содержимого корзины
                var cartItemsContainer = $("#cart-items-container");
                cartItemsContainer.html(data.cart_items_html);
            },
            error: function (data) {
